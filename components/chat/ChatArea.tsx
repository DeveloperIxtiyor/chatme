...
export function ChatArea({ groupId }: ChatAreaProps) {
	...
	return (
		<div className='flex flex-col h-full bg-slate-50 dark:bg-zinc-950 relative'>
						...
						return (
							...
							>
								<div className={`flex items-center gap-2 max-w-[85%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
									<div
										className={`p-3 rounded-2xl break-words shadow-sm cursor-pointer transition-transform active:scale-[0.99] ${
											isMe
									...
									{isMe && (
										<Button
											...
											variant="ghost"
											size="icon"
											className="h-8 w-8 text-destructive hover:bg-destructive/10 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl"
										>
			...
			<form
				onSubmit={handleSend}
				className='p-4 border-t border-border bg-white dark:bg-zinc-900 flex gap-2 items-center shadow-lg'
			>
				<Input
					value={text}
